...
<body>
	...
	<%
		new Date(); // <- 제일 상단에  import 문 사용
		//p.177~
		//내장객체 : JSP 자체적으로 내장되어 있는 객체
		//=> 별도의 객체 생성없이 사용가능 (import문 사용없이도 사용 가능)
		//=> 내장객체는 JSP페이지가 서블릿클래스로 변환될때 _jspService()메서드 안에서
		//	가장 먼저 선언됨.
		
		/* (외우기...)
			[javax.servlet 패키지] - 8개
			 request : 클라이언트의 요청정보를 저장하는 객체(HTTP요청, 파라미터)
			 respense : 요청정보에 대한 응답정보를 처리하는 객체
			 session : 클라이언트의 세션정보를 저장하는 객체
			 pagaContext : 페이지에 실행을 할때 필요한 정보를 저장하는 객체  (프로젝트)
	...
	<hr>
	http 헤더 (user-agent) : <%=request.getHeaders("user-agent") %> <br>
	http 헤더 (accept-language) : <%=request.getHeaders("accept-language") %> <br>
	...
	세션 ID값 : <%=session.getId() %> <br> <!-- 브라우저당 하나가 부여됨, 열때마다 값 바뀜 -->
