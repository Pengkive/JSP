...
<body>
	...
	<h2> 영역객체의 값을 출력 </h2>
	<%
	// 해당영역에 데이터를 저장 -> 해당 영역에서 데이터를 공유해서 사용가능
	// 영역데이터 저장
	...
	1. a 태그 사용<br>
	파라미터값 전달 가능 +
	영역객체 : session, application 정보만 사용가능
	...
	2. javascript 사용 <br>
	파라미터값 전달가능 +
	영역개체 : session, application 정보만 사용가능<br>
...
// 		response.sendRedirect("scopePro2.jsp?id="+id+"&pw=1234");
	...
	4. JSP 액션태그 ( forward 방식 이동)
	포워딩 방식 이동 : 페이지 이동시 request, response 포함해서 이동하는 방식<br>
				A   ->	 B
	페이지 주소 : A	<br>
	페이지 화면 : B <br>
	=> 페이지 주소 변경x, 페이지 화면 변경 o, request, response 가지고 이동 <br>
